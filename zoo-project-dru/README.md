...
helm install my-zoo-project-dru zoo-project/zoo-project-dru --version 0.10.0
````
...
| zoo.processesv2         | Add OGC API - Processes - Part 1: Core 2.0 conformance classes                            | true                   |
...
| zoo.logging.enabled         | Enable logging for the ZOO-Project (ZOO-Kernel and ZOO-FPM)    | false                    |
| zoo.logging.level         | The logging level for the ZOO-Project (ZOO-Kernel and ZOO-FPM)    | "ALL"                    |
...
| workflow.defaultVolumeSize                               | The default volume size allocated with unit (ie. Mi,Gi).  | 10Mi                                                     |
| workflow.defaultMaxRam                                   | The default max ram allocated.      | 1024                                                                  |
| workflow.defaultMaxCores                                 | The default max cores allocated.    | 2                                                                     |
| workflow.calrissianImage                                 | The calrissian image version.       | "ghcr.io/duke-gcb/calrissian/calrissian:latest"                                          |
...
### Toil WES in-cluster (experimental)

In addition to connecting to an external WES endpoint (for example HPC), this chart can optionally deploy a Toil WES server in-cluster.
...
| toilWes.s3.host                                          | S3 endpoint host used by Toil workers | "s3-service" |
| toilWes.s3.port                                          | S3 endpoint port used by Toil workers | "9000" |
| toilWes.s3.useSsl                                        | Use SSL when connecting to S3 endpoint | false |
| toilWes.s3.credentialsSecret.name                        | Secret containing S3 credentials | "s3-service" |
| toilWes.s3.credentialsSecret.accessKeyKey                | Secret key name for access key ID | "rootUser" |
| toilWes.s3.credentialsSecret.secretKeyKey                | Secret key name for secret access key | "rootPassword" |
| toilWes.resolverOptions                                  | DNS resolver options propagated to workers (`RES_OPTIONS`) | "ndots:1" |
| toilWes.env                                              | Extra environment variables for Toil WES | {} |
| toilWes.resources.defaultMemory                          | Default memory assigned to Toil jobs | "8Gi" |
| toilWes.resources.defaultDisk                            | Default disk assigned to Toil jobs | "16Gi" |
| toilWes.resources.maxMemory                              | Optional maximum memory for Toil jobs (empty means no explicit cap) | "" |
| toilWes.resources.maxDisk                                | Optional maximum disk for Toil jobs (empty means no explicit cap) | "" |
